...
class QuickfixConan(ConanFile):
    ...
    def source(self):
        self.run("git clone %s" % self._source_url)
        self.run("cd quickfix && git checkout conan-cmake-support")
        # self.run("cd quickfix && git checkout v%s" % self.version)
        os.rename("quickfix", self._source_subfolder)
